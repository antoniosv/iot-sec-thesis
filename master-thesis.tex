...
\TODO{Replace openHAB2 with openHAB}
...
\subsection{Smarthome Automation Solutions}
...
\subsection{Architecture of Eclipse SmartHome and openHAB2}
...
\TODO{include figures (2)  about authenticators and other resources}

Initial results by the community (REFERENCE) showed that it was indeed possible to make the \texttt{HttpContext} shared by the components that needed access control. Taking these results into account, work in the direction of a \emph{custom} \texttt{HttpContext} started. Implementation details are introduced in the next subsection.

\subsection{Implementation of Authentication}
As an interface class of the \texttt{Http Service} OSGi feature, \texttt{HttpContext} includes a method \texttt{handleSecurity} which, as the name implies, handles the security for the specified request to the servlet. 

\TODO{Sequence diagram on authentication}
